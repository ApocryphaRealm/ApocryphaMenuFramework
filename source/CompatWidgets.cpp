...
// ---- windows / layout ------------------------------------------------------------------------
AMF_EXPORT bool igBegin(const char* a_name, bool* a_open, int a_flags) { return ImGui::Begin(a_name, a_open, a_flags); }
...
AMF_EXPORT bool igBeginPopup(const char* a_id, int a_flags) { return ImGui::BeginPopup(a_id, a_flags); }
AMF_EXPORT bool igBeginPopupModal(const char* a_name, bool* a_open, int a_flags) { return ImGui::BeginPopupModal(a_name, a_open, a_flags); }
AMF_EXPORT void igEndPopup() { ImGui::EndPopup(); }
AMF_EXPORT void igOpenPopup_Str(const char* a_id, int a_flags) { ImGui::OpenPopup(a_id, a_flags); }
AMF_EXPORT void igCloseCurrentPopup() { ImGui::CloseCurrentPopup(); }
AMF_EXPORT bool igBeginCombo(const char* a_label, const char* a_preview, int a_flags) { return ImGui::BeginCombo(a_label, a_preview, a_flags); }
AMF_EXPORT void igEndCombo() { ImGui::EndCombo(); }
AMF_EXPORT bool igBeginTabBar(const char* a_id, int a_flags) { return ImGui::BeginTabBar(a_id, a_flags); }
AMF_EXPORT void igEndTabBar() { ImGui::EndTabBar(); }
AMF_EXPORT bool igBeginTabItem(const char* a_label, bool* a_open, int a_flags) { return ImGui::BeginTabItem(a_label, a_open, a_flags); }
...
// ---- tables ---------------------------------------------------------------------------------
AMF_EXPORT bool igBeginTable(const char* a_id, int a_columns, int a_flags, const ImVec2 a_outer, float a_innerWidth) { return ImGui::BeginTable(a_id, a_columns, a_flags, a_outer, a_innerWidth); }
AMF_EXPORT void igEndTable() { ImGui::EndTable(); }
AMF_EXPORT void igTableSetupColumn(const char* a_label, int a_flags, float a_initWidth, ImGuiID a_userId) { ImGui::TableSetupColumn(a_label, a_flags, a_initWidth, a_userId); }
...
// ---- text / widgets ----------------------------------------------------------------------------
AMF_EXPORT void igBulletTextV(const char* a_fmt, va_list a_args) { ImGui::BulletTextV(a_fmt, a_args); }
AMF_EXPORT void igTextColoredV(const ImVec4 a_col, const char* a_fmt, va_list a_args) { ImGui::TextColoredV(a_col, a_fmt, a_args); }
AMF_EXPORT void igTextUnformatted(const char* a_text, const char* a_end) { ImGui::TextUnformatted(a_text, a_end); }
...
AMF_EXPORT bool igDragFloat(const char* a_label, float* a_v, float a_speed, float a_min, float a_max, const char* a_fmt, int a_flags) { return ImGui::DragFloat(a_label, a_v, a_speed, a_min, a_max, a_fmt, a_flags); }
AMF_EXPORT bool igInputFloat(const char* a_label, float* a_v, float a_step, float a_stepFast, const char* a_fmt, int a_flags) { return ImGui::InputFloat(a_label, a_v, a_step, a_stepFast, a_fmt, a_flags); }
AMF_EXPORT bool igInputTextWithHint(const char* a_label, const char* a_hint, char* a_buf, size_t a_size, int a_flags, ImGuiInputTextCallback a_cb, void* a_user) { return ImGui::InputTextWithHint(a_label, a_hint, a_buf, a_size, a_flags, a_cb, a_user); }
AMF_EXPORT bool igRadioButton_IntPtr(const char* a_label, int* a_v, int a_button) { return ImGui::RadioButton(a_label, a_v, a_button); }
AMF_EXPORT bool igSmallButton(const char* a_label) { return ImGui::SmallButton(a_label); }
...
AMF_EXPORT bool igTreeNodeEx_Str(const char* a_label, int a_flags) { return ImGui::TreeNodeEx(a_label, a_flags); }
