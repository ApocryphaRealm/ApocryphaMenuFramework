#include "Registry.h"

#include "utils/Logger.h"

#include <mutex>

namespace registry
{
	namespace
	{
		std::mutex g_lock;
		std::vector<Entry> g_entries;
	}

	bool Register(const char* a_modName, const char* a_pageName, AMF_RenderCallback a_render)
	{
		if (!a_modName || !*a_modName || !a_pageName || !*a_pageName || !a_render)
		{
			logger::warn("AMF_RegisterPage refused: mod=\"{}\", page=\"{}\", render={} - every argument must be non-null/non-empty",
						 a_modName ? a_modName : "<null>", a_pageName ? a_pageName : "<null>",
						 static_cast<const void*>(reinterpret_cast<void*>(a_render)));
			return false;
		}

		std::scoped_lock lock(g_lock);

		for (auto& entry : g_entries)
		{
			if (entry.modName == a_modName)
			{
				for (const auto& page : entry.pages)
				{
					if (page.pageName == a_pageName)
					{
						logger::warn("AMF_RegisterPage refused: \"{}\" already has a page \"{}\" (duplicate registration)",
									 a_modName, a_pageName);
						return false;
					}
				}

				entry.pages.push_back({ a_pageName, a_render });
				logger::info("page registered: \"{}\" -> \"{}\" (mod now has {} page(s), rendered as tabs - one menu per mod)",
							 a_modName, a_pageName, entry.pages.size());
				return true;
			}
		}

		g_entries.push_back({ a_modName, { { a_pageName, a_render } } });
		logger::info("first page registered for \"{}\": \"{}\" ({} mod(s) in the registry)",
					 a_modName, a_pageName, g_entries.size());
		return true;
	}

	std::vector<Entry> Snapshot()
	{
		std::scoped_lock lock(g_lock);
		return g_entries;
	}

	std::size_t Count()
	{
		std::scoped_lock lock(g_lock);
		return g_entries.size();
	}
}

// HUD widgets (1.4.4) - same locking discipline as the pages: registration from any thread,
// snapshot on the render thread, callbacks never run under the lock.
namespace registry
{
	namespace
	{
		std::mutex g_hudLock;
		std::vector<HudWidget> g_hud;
	}

	bool RegisterHud(const char* a_modName, const char* a_widgetName, AMF_HudCallback a_draw)
	{
		if (!a_modName || !*a_modName || !a_widgetName || !*a_widgetName || !a_draw)
		{
			logger::warn("AMF_RegisterHudWidget refused: mod=\"{}\", widget=\"{}\", draw={} - every argument must be non-null/non-empty",
						 a_modName ? a_modName : "<null>", a_widgetName ? a_widgetName : "<null>",
						 static_cast<const void*>(reinterpret_cast<void*>(a_draw)));
			return false;
		}
		std::scoped_lock lock(g_hudLock);
		for (const auto& w : g_hud)
		{
			if (w.modName == a_modName && w.widgetName == a_widgetName)
			{
				logger::warn("AMF_RegisterHudWidget refused: \"{}\" already registered widget \"{}\"", a_modName, a_widgetName);
				return false;
			}
		}
		g_hud.push_back({ a_modName, a_widgetName, a_draw });
		logger::info("HUD widget registered: \"{}\" -> \"{}\" ({} widget(s) total)", a_modName, a_widgetName, g_hud.size());
		return true;
	}

	std::vector<HudWidget> HudSnapshot()
	{
		std::scoped_lock lock(g_hudLock);
		return g_hud;
	}
}
